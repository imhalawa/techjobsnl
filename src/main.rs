...
use job_watch::{
    ...
    sources::{
        ...
        pay, personio, pggm, postnl, rabobank, recruitee, successfactors, successfactors_api, uber,
        ...
        .map(|company| -> Result<Arc<dyn JobSource>> {
            match &company.source {
                ...
                SourceConfig::Workday {
                    ...
                    country,
                    country_code,
                } => Ok(Arc::new(workday::WorkdaySource::new(
                    &company.id,
                    base_url,
                    tenant,
                    site,
                ...
                SourceConfig::Exact { listing_url } => Ok(Arc::new(exact::ExactSource::new(
                    &company.id,
                    listing_url,
                    client.clone(),
                ))),
                SourceConfig::Afas { listing_url } => Ok(Arc::new(afas::AfasSource::new(
                    &company.id,
                    listing_url,
                    client.clone(),
                ))),
                SourceConfig::Ns { listing_url } => Ok(Arc::new(ns::NsSource::new(
                    &company.id,
                    listing_url,
                    client.clone(),
                ))),
                ...
                SourceConfig::Postnl { api_url } => Ok(Arc::new(postnl::PostnlSource::new(
                    &company.id,
                    api_url,
                    client.clone(),
                ))),
                SourceConfig::Pggm { listing_url } => Ok(Arc::new(pggm::PggmSource::new(
                    &company.id,
                    listing_url,
                    client.clone(),
                ))),
                SourceConfig::Amazon { search_url } => Ok(Arc::new(amazon::AmazonSource::new(
                    &company.id,
                    search_url,
                    client.clone(),
                ))),
                SourceConfig::Uber { api_url } => Ok(Arc::new(uber::UberSource::new(
                    &company.id,
                    api_url,
                    client.clone(),
                ))),
                ...
                SourceConfig::Successfactors {
                    listing_url,
                    employer,
                } => Ok(Arc::new(successfactors::SuccessFactorsSource::new(
                    &company.id,
                    listing_url,
                    employer,
                    client.clone(),
                ...
                SourceConfig::SuccessfactorsApi { base_url } => {
                    Ok(Arc::new(successfactors_api::SuccessFactorsSource::new(
                        &company.id,
                        &company.name,
                        base_url,
                        client.clone(),
...
mod tests {
    ...
    fn startup_refreshes_the_company_catalog_without_losing_user_configuration() {
        ...
        assert_eq!(migrated.companies.len(), 67);
        assert!(
            migrated
                .companies
                .iter()
